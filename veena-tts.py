...
def decode_snac_tokens(snac_tokens, snac_model):
    ...
    hierarchical_codes = []
#    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')  # Explicit device
    device = 'cuda'
...
text_english = "Welcome to cultYvate... This is a verification call... Please press one if you are Lucky Singh Son of Haripreeth Singh. If not, press two."
audio = generate_speech(text_english, speaker="maitri")
sf.write("cultyuvate2_english_agastya.wav", audio, 24000)
endtime = datetime.now()
total_time = (endtime - start_time).total_seconds()
print(f"Total time taken to generate the text is : {total_time:.2f} seconds")
